...
CFLAGS_klipper.elf = $(CFLAGS) -Wl,--gc-sections,-Map,$(OUT)klipper.map
...
board_type := bed
else ifeq ($(CONFIG_EXT_MCU_BOARD),y)
board_type := ext
endif

ifneq ($(CONFIG_CREALITY_MCU_MENU),)
mcu_menu := $(patsubst "%",%,$(CONFIG_CREALITY_MCU_MENU))
else
mcu_menu :=
endif

ifneq ($(CONFIG_CREALITY_MCU_TYPE),)
mcu_type := $(patsubst "%",%,$(CONFIG_CREALITY_MCU_TYPE))
else
mcu_type :=
endif
...
$(OUT)$(rename_bin): $(OUT)klipper.bin $(OUT)hostCrc16.elf
	@echo "  Adding CRC"
	$(Q)$(OUT)hostCrc16.elf $<
