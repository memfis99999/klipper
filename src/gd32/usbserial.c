...
/*!< usb endpoint 0 tx buffer address offset */
#define EPT0_TX_ADDR                     0x40
/*!< usb endpoint 0 rx buffer address offset */
#define EPT0_RX_ADDR                     0x80
   /*!< usb endpoint 1 tx buffer address offset */
#define EPT1_TX_ADDR                     0xC0
  /*!< usb endpoint 1 rx buffer address offset */
#define EPT1_RX_ADDR                     0x100
/*!< usb endpoint 2 tx buffer address offset */
...
USBFS_L_CAN1_RX0_IRQHandler(void)
{
    switch(USB->intsts_bit.pet_num)
    {
        case 0:
                ...
                {
                    //ep0 out event
                    if((USB->ept_bit[0].rxtc == 1) &&
                       (USB->ept_bit[0].setuptc == 0))
                    {
                        //ep0 out event
                    }
                    else
...
usbserial_init(void)
{
    ...
    *(uint32_t *)((USB->buftbl + 4) * 2 + USB_PACKET_BUFFER_ADDRESS) =
        EPT0_RX_ADDR;
